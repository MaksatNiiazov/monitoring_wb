...
from monitoring.views import _build_stock_popup_payload, _table_row_visual_key
...
class ReportingTests(TestCase):
    ...
    def test_product_report_uses_product_keyword_registry_when_daily_note_is_empty(self) -> None:
        ProductKeyword.objects.create(product=self.product, query_text="alpha keyword", position=0)
        DailyProductKeywordStat.objects.create(
            product=self.product,
            stats_date=date(2026, 3, 16),
            query_text="alpha keyword",
            frequency=33,
            organic_position=Decimal("7.00"),
        )
        ...
        self.assertEqual(report["keyword_rows"][0]["query_text"], "alpha keyword")
        self.assertEqual(report["keyword_rows"][0]["frequency"], 33)
    ...
    def test_export_contains_sample_header(self) -> None:
        ...
        self.assertEqual(rows[4][2], "2000")
        self.assertEqual(rows[4][7], "2000")
    ...
    def test_exporter_uses_overall_funnel_in_summary_column(self) -> None:
        ...
        self.assertEqual(rows[12][7], "10")
        self.assertEqual(rows[14][7], "10000")
    ...
    def test_exporter_normalizes_buyout_percent_when_saved_as_fraction(self) -> None:
        ...
        self.assertEqual(rows[15][7], "2400")
    ...
    def test_exporter_populates_overall_ctr_cpm_and_cpc(self) -> None:
        ...
        rows = exporter_rows(report)
        self.assertEqual(rows[6][7], "10%")
        self.assertEqual(rows[7][7], "2000")
        self.assertEqual(rows[8][7], "20")
    ...
    def test_exporter_uses_dash_for_unavailable_derived_metrics(self) -> None:
        ...
        rows = exporter_rows(report)
        self.assertEqual(rows[8][2], "-")
        self.assertEqual(rows[11][7], "-")
        self.assertEqual(rows[13][7], "-")
        self.assertEqual(rows[16][7], "-")
        self.assertEqual(rows[17][7], "-")
        self.assertEqual(rows[18][7], "-")
        self.assertEqual(rows[19][7], "-")
    ...
    def test_exporter_keeps_revenue_and_buyouts_but_hides_efficiency_metrics_when_spend_is_zero(self) -> None:
        ...
        self.assertEqual(rows[14][2], "6000")
        self.assertEqual(rows[15][2], "1440")
        self.assertEqual(rows[16][2], "-")
        self.assertEqual(rows[17][2], "-")
        self.assertEqual(rows[18][2], "-")
        self.assertEqual(rows[19][2], "-")
        self.assertEqual(rows[14][7], "10000")
        self.assertEqual(rows[15][7], "2400")
        self.assertEqual(rows[16][7], "-")
        self.assertEqual(rows[17][7], "-")
    ...
    def test_exporter_shows_tiny_nonzero_percentages_without_rounding_to_zero(self) -> None:
        ...
        self.assertEqual(rows[18][2], "<0,01%")
        self.assertEqual(rows[19][2], "<0,01%")
        self.assertEqual(rows[18][7], "<0,01%")
        self.assertEqual(rows[19][7], "<0,01%")
    ...
    def test_product_report_contains_insights_and_zone_cards(self) -> None:
        ...
        self.assertEqual(len(report["traffic_cards"]), 6)
    ...
    def test_product_report_keeps_unified_campaign_in_shelves_when_cluster_stats_exist(self) -> None:
        ...
        self.assertEqual(unified_search.impressions, 0)
        self.assertEqual(unified_search.clicks, 0)
        self.assertEqual(unified_search.spend, Decimal("0"))
        self.assertEqual(unified_search.carts, 0)
        self.assertEqual(unified_search.orders, 0)
        self.assertEqual(unified_search.order_sum, Decimal("0"))

        self.assertEqual(unified_shelves.impressions, 1000)
        self.assertEqual(unified_shelves.clicks, 100)
        self.assertEqual(unified_shelves.spend, Decimal("2000.00"))
        self.assertEqual(unified_shelves.carts, 20)
        self.assertEqual(unified_shelves.orders, 6)
        self.assertEqual(unified_shelves.order_sum, Decimal("6000.00"))
    ...
    def test_product_report_keeps_catalog_zone_when_cluster_stats_exist(self) -> None:
        ...
        self.assertEqual(unified_search.orders, 0)
        self.assertEqual(unified_search.order_sum, Decimal("0"))
        self.assertEqual(unified_shelves.impressions, 4500)
        self.assertEqual(unified_shelves.clicks, 270)
        self.assertEqual(unified_shelves.spend, Decimal("4300.00"))
        self.assertEqual(unified_shelves.carts, 38)
        self.assertEqual(unified_shelves.orders, 9)
        self.assertEqual(unified_shelves.order_sum, Decimal("8700.00"))
        ...
        self.assertEqual(unified_search.clicks + unified_shelves.clicks + unified_catalog.clicks, 270)
        self.assertEqual(unified_search.carts + unified_shelves.carts + unified_catalog.carts, 38)
        self.assertEqual(unified_search.orders + unified_shelves.orders + unified_catalog.orders, 9)
        self.assertEqual(
            unified_search.spend + unified_shelves.spend + unified_catalog.spend,
            Decimal("4300.00"),
        )
        self.assertEqual(
            unified_search.order_sum + unified_shelves.order_sum + unified_catalog.order_sum,
            Decimal("8700.00"),
        )

    def test_product_report_splits_manual_search_catalog_from_search_clusters(self) -> None:
        manual_search_campaign = Campaign.objects.create(
            external_id=28150165,
            name="Manual search split",
        ...
        manual_search = report["blocks"]["manual_search"]
        manual_catalog = report["blocks"]["manual_catalog"]
        self.assertEqual(manual_search.impressions, 700)
        self.assertEqual(manual_search.clicks, 70)
        self.assertEqual(manual_search.spend, Decimal("700.00"))
        self.assertEqual(manual_search.carts, 35)
        self.assertEqual(manual_search.orders, 7)
        self.assertEqual(manual_search.order_sum, Decimal("7000.00"))
        self.assertEqual(manual_catalog.impressions, 300)
        self.assertEqual(manual_catalog.clicks, 30)
        self.assertEqual(manual_catalog.spend, Decimal("300.00"))
        self.assertEqual(manual_catalog.carts, 15)
        self.assertEqual(manual_catalog.orders, 3)
        self.assertEqual(manual_catalog.order_sum, Decimal("3000.000"))

    def test_product_report_uses_campaign_totals_with_item_catalog_residual(self) -> None:
        manual_search_campaign = Campaign.objects.create(
            external_id=28150166,
            name="Manual search campaign total split",
            monitoring_group=CampaignMonitoringGroup.MANUAL_SEARCH,
        )
        manual_search_campaign.products.add(self.product)
        DailyCampaignProductStat.objects.create(
            ...
            clicks=100,
            spend=Decimal("1000.00"),
            add_to_cart_count=80,
            order_count=20,
            units_ordered=20,
            order_sum=Decimal("20000.00"),
            raw_payload={
                "items": [
                    {
                        "appType": 32,
        ...
        self.assertEqual(manual_search.clicks, 70)
        self.assertEqual(manual_search.spend, Decimal("700.00"))
        self.assertEqual(manual_search.carts, 65)
        self.assertEqual(manual_search.orders, 17)
        self.assertEqual(manual_search.order_sum, Decimal("17000.00"))
        self.assertEqual(manual_catalog.impressions, 300)
        self.assertEqual(manual_catalog.clicks, 30)
        self.assertEqual(manual_catalog.spend, Decimal("300.00"))
        self.assertEqual(manual_catalog.carts, 15)
        self.assertEqual(manual_catalog.orders, 3)
        self.assertEqual(manual_catalog.order_sum, Decimal("3000.00"))
    ...
    def test_monitoring_day_block_profit_formula_uses_drr_sales_fraction(self) -> None:
        ...
        self.assertIn("IF(H20>1,H20/100,H20)", block[20][1])
    ...
    def test_monitoring_day_block_uses_safe_divide_formulas_for_derived_metrics(self) -> None:
        ...
        block = build_day_block(report, start_row=1, start_col=1)
        self.assertTrue(block[11][7].startswith("=IFERROR(IF("))
        self.assertTrue(block[13][7].startswith("=IFERROR(IF("))
        self.assertTrue(block[16][2].startswith("=IFERROR(IF("))
        self.assertTrue(block[17][2].startswith("=IFERROR(IF("))
        self.assertTrue(block[18][2].startswith("=IFERROR(IF("))
        self.assertTrue(block[19][2].startswith("=IFERROR(IF("))
    ...
    def test_monitoring_day_block_hides_efficiency_metrics_when_spend_is_zero(self) -> None:
        ...
        self.assertIn('C5=0', block[16][2])
        self.assertIn('C5=0', block[17][2])
        self.assertIn('C5=0', block[18][2])
        self.assertIn('C5=0', block[19][2])
        self.assertIn('H5=0', block[16][7])
        self.assertIn('H5=0', block[17][7])
        self.assertIn('H5=0', block[18][7])
        self.assertIn('H5=0', block[19][7])
    ...
    def test_monitoring_day_block_traffic_share_matches_template_logic(self) -> None:
        ...
        self.assertEqual(traffic_row[1], "")
        self.assertEqual(traffic_row[2], 1.0)
        self.assertEqual(traffic_row[3], "")
        self.assertEqual(traffic_row[4], 1.0)
        self.assertEqual(traffic_row[5], "")
        self.assertEqual(traffic_row[6], "")
        self.assertEqual(traffic_row[8], "-")
    ...
    def test_exporter_traffic_share_matches_template_logic(self) -> None:
        ...
        self.assertEqual(traffic_row[1], "")
        self.assertEqual(traffic_row[2], "100%")
        self.assertEqual(traffic_row[3], "")
        self.assertEqual(traffic_row[4], "100%")
        self.assertEqual(traffic_row[5], "")
        self.assertEqual(traffic_row[6], "")
        self.assertEqual(traffic_row[7], "100%")
        self.assertEqual(traffic_row[8], "-")

    def test_table_blocks_keep_manual_search_separate_from_manual_shelves(self) -> None:
        manual_search_campaign = Campaign.objects.create(
            external_id=28150157,
            name="Manual search",
            monitoring_group=CampaignMonitoringGroup.MANUAL_SEARCH,
        ...
        DailyProductMetrics.objects.filter(
            product=self.product,
            stats_date=date(2026, 3, 16),
        ).update(open_count=220, add_to_cart_count=40, order_count=12, order_sum=Decimal("12000.00"))
        DailyCampaignProductStat.objects.create(
            campaign=manual_search_campaign,
            product=self.product,
            stats_date=date(2026, 3, 16),
        ...
        self.assertEqual(report["table_blocks"]["manual"].clicks, 15)
        self.assertEqual(report["table_blocks"]["manual"].spend, Decimal("300.00"))
        self.assertEqual(report["table_blocks"]["shelves"].clicks, 112)
        self.assertEqual(report["table_blocks"]["shelves"].spend, Decimal("2260.00"))
        self.assertEqual(report["table_organic"]["open_count"], 93)
        self.assertEqual(report["table_organic"]["cart_count"], 15)
        self.assertEqual(report["table_organic"]["order_count"], 4)
        self.assertEqual(report["table_organic"]["order_sum"], Decimal("4800.00"))
...
class SyncTests(TestCase):
    ...
    def test_aggregate_campaign_day_stats_uses_app_totals_for_single_linked_product(self) -> None:
        payload = {
            "apps": [
                {
                    "appType": 32,
        ...
        aggregated = _aggregate_campaign_day_stats(
            payload,
            campaign_group=CampaignMonitoringGroup.UNIFIED,
            product_map={self.product.nm_id: self.product},
            linked_product_ids={self.product.id},
        )

        row = aggregated[(self.product.id, CampaignZone.RECOMMENDATION)]
        self.assertEqual(row["impressions"], 110)
        self.assertEqual(row["clicks"], 12)
        self.assertEqual(row["spend"], Decimal("60.00"))
        self.assertEqual(row["add_to_cart_count"], 7)
        self.assertEqual(row["order_count"], 3)
        self.assertEqual(row["units_ordered"], 4)
        self.assertEqual(row["order_sum"], Decimal("5000.00"))
        self.assertEqual(row["raw_payload"][0]["item"]["atbs"], 2)
...
class PreparationTests(TestCase):
    ...
    def test_table_row_visual_key_uses_exact_labels(self) -> None:
        self.assertEqual(_table_row_visual_key("Клики"), "")
        self.assertEqual(_table_row_visual_key("Корзины"), "carts")
        self.assertEqual(_table_row_visual_key("Заказы"), "orders")
        self.assertEqual(_table_row_visual_key("Заказы (руб.)"), "")
        self.assertEqual(_table_row_visual_key("Конверсия в заказ (%)"), "conversion-order")
...
class PageRenderTests(TestCase):
    ...
    def test_table_page_renders_monitoring_grid(self) -> None:
        seed_demo_dataset()
        response = self.client.get("/table/?sheet=sheet-1")
        self.assertEqual(response.status_code, 200)
    ...
    def test_campaign_detail_page_renders_stats_workspace(self) -> None:
        ...
        self.assertContains(response, 'data-detail-tab-trigger="products"')
...
class TableInlineNoteUpdateTests(TestCase):
    ...
    def test_table_workspace_renders_keyword_controls(self) -> None:
        response = self.client.get("/table/?sheet=sheet-1&history_days=1&reference_date=2026-03-18")

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'data-field="keyword_rows_count_delta"')
        self.assertContains(response, 'data-field="keyword_query"')
        self.assertContains(response, 'data-note-control="keyword-delete"')
        self.assertNotContains(response, 'data-delta="-1"')
        self.assertNotContains(response, "is-keyword-label-col")

    def test_table_workspace_keeps_keyword_names_in_first_column_across_days(self) -> None:
        response = self.client.get("/table/?sheet=sheet-1&history_days=2&reference_date=2026-03-18")

        self.assertEqual(response.status_code, 200)
        html = response.content.decode("utf-8")
        self.assertEqual(html.count('data-field="keyword_query"'), 3)
        self.assertEqual(html.count('data-field="keyword_frequency"'), 6)

    def test_table_workspace_aligns_overview_controls_after_keywords(self) -> None:
        response = self.client.get("/table/?sheet=sheet-1&history_days=1&reference_date=2026-03-18")
        ...
        def row_html(label: str) -> str:
            ...
            row_start = html.rfind("<tr", 0, label_index)
            row_end = html.find("</tr>", label_index)
            return html[row_start:row_end]

        self.assertNotIn('data-field=', row_html("Обзор:"))
        self.assertIn('colspan="8"', row_html("Обзор:"))
        self.assertIn('data-field="spp_percent"', row_html("СПП"))
        self.assertIn('colspan="5"', row_html("СПП"))
        self.assertIn('data-field="seller_price"', row_html("Цена WBSELLER (наша)"))
        self.assertIn('colspan="2"', row_html("Цена WBSELLER (наша)"))
        self.assertIn('data-field="wb_price"', row_html("Цена WB (на сайте)"))
        self.assertIn('data-field="promo_status"', row_html("Акция"))
        self.assertIn('data-field="negative_feedback"', row_html("Негативные отзывы"))
        self.assertNotIn('data-field=', row_html("Действия:"))
        self.assertIn('colspan="8"', row_html("Действия:"))
        self.assertIn('data-field="unified_enabled"', row_html("Включили РК единая?"))
        self.assertIn('data-field="manual_search_enabled"', row_html("Включили РК руч.поиск"))
        self.assertIn('data-field="price_changed"', row_html("Меняли цену?(WBSeller)"))
        self.assertNotIn('data-field=', row_html("Комментарии:"))
        self.assertIn('colspan="8"', row_html("Комментарии:"))
    ...
    def test_table_note_cell_syncs_added_keyword_to_other_notes(self) -> None:
        other_note = DailyProductNote.objects.create(
            product=self.product,
            note_date=date(2026, 3, 17),
            keyword_rows_count=1,
        )
        self.note.keyword_rows_count = 1
        self.note.save(update_fields=["keyword_rows_count", "updated_at"])
        ...
        self.assertEqual(self.note.keywords, ["delta keyword"])
        self.assertEqual(other_note.keywords, ["delta keyword"])
        self.assertEqual(self.note.keyword_rows_count, 1)
        self.assertEqual(other_note.keyword_rows_count, 1)

    def test_table_note_cell_deletes_specific_keyword_row(self) -> None:
        other_note = DailyProductNote.objects.create(
            product=self.product,
            note_date=date(2026, 3, 17),
        ...
        self.note.keywords = ["alpha", "beta", "gamma"]
        self.note.keyword_rows_count = 3
        self.note.save(update_fields=["keywords", "keyword_rows_count", "updated_at"])
        ProductKeyword.objects.create(product=self.product, query_text="alpha", position=0)
        ProductKeyword.objects.create(product=self.product, query_text="beta", position=1)
        ProductKeyword.objects.create(product=self.product, query_text="gamma", position=2)
        DailyProductKeywordStat.objects.create(
            product=self.product,
            stats_date=date(2026, 3, 18),
            query_text="beta",
        ...
        self.assertEqual(other_note.keyword_rows_count, 2)
        self.assertEqual(self.note.keywords, ["alpha", "gamma"])
        self.assertEqual(other_note.keywords, ["alpha", "gamma"])
        self.assertEqual(
            list(
                ProductKeyword.objects.filter(product=self.product)
                .order_by("position")
    ...
    def test_table_note_cell_deletes_filled_keyword_row_without_collapsing_extra_blank_rows(self) -> None:
        other_note = DailyProductNote.objects.create(
            product=self.product,
            note_date=date(2026, 3, 17),
            keywords=["alpha"],
        ...
        self.assertEqual(self.note.keyword_rows_count, 2)
        self.assertEqual(other_note.keyword_rows_count, 2)
        self.assertEqual(self.note.keywords, [])
        self.assertEqual(other_note.keywords, [])
        self.assertFalse(ProductKeyword.objects.filter(product=self.product).exists())

    def test_table_note_cell_deletes_empty_keyword_row(self) -> None:
        other_note = DailyProductNote.objects.create(
            product=self.product,
            note_date=date(2026, 3, 17),
            keywords=["alpha"],
        ...
        self.assertEqual(self.note.keyword_rows_count, 2)
        self.assertEqual(other_note.keyword_rows_count, 2)
        self.assertEqual(self.note.keywords, ["alpha"])
        self.assertEqual(other_note.keywords, ["alpha"])
        self.assertTrue(ProductKeyword.objects.filter(product=self.product, query_text="alpha").exists())

    def test_table_note_cell_updates_registered_keyword_from_empty_note(self) -> None:
        ProductKeyword.objects.create(product=self.product, query_text="alpha keyword", position=0)
        ProductKeyword.objects.create(product=self.product, query_text="beta keyword", position=1)
        ...
        self.assertEqual(response.status_code, 200)
        self.note.refresh_from_db()
        self.assertEqual(self.note.keywords, ["alpha keyword", "gamma keyword"])
        self.assertEqual(
            list(
                ProductKeyword.objects.filter(product=self.product)
                .order_by("position")
    ...
    def test_table_note_cell_allows_keyword_rows_below_three(self) -> None:
        other_note = DailyProductNote.objects.create(
            product=self.product,
            note_date=date(2026, 3, 17),
            keyword_rows_count=3,
        ...
        self.note.refresh_from_db()
        other_note.refresh_from_db()
        self.assertEqual(self.note.keyword_rows_count, 0)
        self.assertEqual(other_note.keyword_rows_count, 0)
